...
const sk = @cImport({
    @cInclude("sokol.h");
    @cInclude("shaders/noninterleaved.glsl.h");
});
...
export fn init() void {
    ...
    const shd = sk.sg_make_shader(sk.noninterleaved_shader_desc(sk.sg_query_backend()));
    ...
    pip_desc.layout.attrs[0] = .{.format = sk.SG_VERTEXFORMAT_FLOAT3, .buffer_index = 0};
    pip_desc.layout.attrs[1] = .{.format = sk.SG_VERTEXFORMAT_FLOAT4, .buffer_index = 1};
    pip = sk.sg_make_pipeline(&pip_desc);

    bind.vertex_buffers[0] = vbuf;
    bind.vertex_buffers[1] = vbuf;
